...
* **Opt-in NPC model** - Only Hytale-namespace NPCs are auto-seeded with nameplates. Other mods call `NameplateAPI.register()` to opt in, preventing conflicts
...
* **PlaceholderAPI support** - Optional integration with PlaceholderAPI. Segment text containing `%placeholder%` tokens is parsed per-viewer with a 500ms TTL cache
