...
By default, your resolver runs **every tick** (20 times per second). For health or mana that changes constantly, that's exactly what you want. But for data that rarely changes - like a faction name, level, or title - recomputing every tick is wasteful.
...
NameplateAPI.define(this, "faction", "Faction",
    ...
    .cacheTicks(100) // recompute every 100 ticks (~5 seconds at 20 tps)
    .resolver((store, entityRef, variantIndex) -> {
