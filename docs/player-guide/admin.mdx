...
Same editor as the player view, but changes are saved to the admin chain (UUID `0`). When the chain is locked, all players see this exact order.
...
All admin configuration is stored in `admin_config.txt` in the mod's data folder. See the [Persistence reference](/reference/persistence) for the file format.
